...
public partial class Display : Form {
	...
	private void RefreshDgv(List<DirectoryInfo>? folders = null) {
		if (folders is null) {
			DirectoryInfo backupFolder = new(BackupFolderTextbox.Text);
			var saveFiles = backupFolder.GetFiles("*.lsv", SearchOption.AllDirectories);
			folders = saveFiles
				.Select(f => f.Directory!)
				.OrderByDescending(d => d.LastWriteTime)
				.ToList();
			var normalSaves = folders.Where(f => !f.Name.Contains("_HonourMode"));
			var honorSaves = folders.Where(f => f.Name.Contains("_HonourMode"));
			if (HonorOnly.Checked)
				folders = honorSaves.ToList(); //folders = null;
			//else
		...
		if (SavesDgv.InvokeRequired) {
			...
			foreach (var folder in folders!) {
				string file = folder.FullName.Replace(BackupFolderTextbox.Text, "");
				if (!folder.Exists)
					continue;
				if (file.StartsWith('\\'))
